...
<div class="flex min-h-screen -mt-8 -mx-4 sm:-mx-6 lg:-mx-8" style="background: linear-gradient(135deg, rgba(241, 245, 249, 0.85) 0%, rgba(226, 232, 240, 0.88) 100%), url('{{ asset('assets/img/service_callcenter_1786525651775.jpg') }}') center/cover fixed no-repeat;">
    ...
    <div class="flex-1 p-6 md:p-10 overflow-y-auto">
        ...
        <div class="mb-8 rounded-2xl p-8 text-white shadow-xl flex flex-col md:flex-row md:items-center justify-between gap-6 relative overflow-hidden"
             style="background: linear-gradient(135deg, rgba(209, 17, 65, 0.88) 0%, rgba(163, 14, 50, 0.92) 100%), url('{{ asset('assets/img/service_callcenter_1786525651775.jpg') }}') center/cover no-repeat;">
            ...
            <a href="{{ route('callcenter.index') }}" target="_blank"
               class="shrink-0 inline-flex items-center gap-2 bg-white hover:bg-red-50 text-red-700 font-black text-xs px-4 py-2.5 rounded-xl shadow transition-all relative z-10">
                ...
                <table class="min-w-full divide-y divide-slate-200">
                    <thead class="bg-slate-50">
                        <tr>
                            ...
                            <th scope="col" class="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Pièce jointe</th>
                            ...
                            <tr class="transition-colors duration-150 {{ $request->status === 'Traité' ? 'bg-emerald-100/60 hover:bg-emerald-200/60' : ($request->status === 'En cours' ? 'bg-blue-100/60 hover:bg-blue-200/60' : 'hover:bg-slate-50') }}">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm">
                                    @if($request->attachment)
                                        <a href="{{ asset('storage/' . $request->attachment) }}" target="_blank" class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold text-blue-700 bg-blue-50 border border-blue-200 hover:bg-blue-100 transition-colors">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    @else
                                        <span class="text-slate-400 text-xs italic">Aucune</span>
                                    @endif
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
